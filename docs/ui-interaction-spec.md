...
### 步骤四：活动生成
* 确认后生成活动对象
* 页面来源标签自动标记为「上传记录」；分享资产保留 `UPLOADED`
* 引导进入分享素材生成
...
| 标签 | 样式建议 | 颜色 |
|---|---|---|
| 页面「GPS 实测」 / 分享资产 `GPS VERIFIED` | 页面使用中文文案；分享海报 / share-render 保留品牌 logo + 分隔线 + ✓ + 英文 | 深绿底 + 薄荷绿边框 |
| 页面「上传记录」 / 分享资产 `UPLOADED` | 页面使用中文文案；分享海报 / share-render 保留文档 icon + 英文 | 深灰底 + 灰色边框 |
...
* 页面 UI 标签文案必须使用「GPS 实测」/「上传记录」
* 分享海报 / share-render / 字体子集保留英文 `GPS VERIFIED` / `UPLOADED`
* 标签使用 `label-s` 字号
* 标签使用 `radius-xs` 圆角
* 标签不抢占主视觉层级
* 分享资产 `GPS VERIFIED` 标签稍带信任感色彩
* 分享资产 `UPLOADED` 标签为中性色，不带贬义
