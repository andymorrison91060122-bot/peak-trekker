-- P0 route truthfulness: keep raw points, apply shared movement deadband only to metrics.
...
DECLARE
  ...
  metric_accuracy DOUBLE PRECISION;
  metric_threshold_m DOUBLE PRECISION;
...
BEGIN
  ...
  FOREACH raw_point IN ARRAY stored_points LOOP
    ...
    IF metric_anchor IS NULL THEN
      IF point_accuracy <= 50 THEN
        metric_anchor := raw_point;
      END IF;
      CONTINUE;
    END IF;

    metric_accuracy := greatest(
      (metric_anchor ->> 'accuracy')::DOUBLE PRECISION,
      (raw_point ->> 'accuracy')::DOUBLE PRECISION
    );
    IF metric_accuracy > 50 THEN
      CONTINUE;
    END IF;

    prev_lat := (metric_anchor ->> 'lat')::DOUBLE PRECISION;
    prev_lng := (metric_anchor ->> 'lng')::DOUBLE PRECISION;
    point_lat := (raw_point ->> 'lat')::DOUBLE PRECISION;
    point_lng := (raw_point ->> 'lng')::DOUBLE PRECISION;
    d_lat := radians(point_lat - prev_lat);
    d_lng := radians(point_lng - prev_lng);
    hav_a := sin(d_lat / 2) * sin(d_lat / 2)
      + cos(radians(prev_lat)) * cos(radians(point_lat)) * sin(d_lng / 2) * sin(d_lng / 2);
    segment_m := 6371000 * 2 * atan2(sqrt(hav_a), sqrt(greatest(0, 1 - hav_a)));
    metric_threshold_m := greatest(8, metric_accuracy);
    IF segment_m < metric_threshold_m THEN
      CONTINUE;
    END IF;

    total_distance_m := total_distance_m + segment_m;

    IF metric_anchor ? 'altitude'
      AND metric_anchor ->> 'altitude' IS NOT NULL
      AND raw_point ? 'altitude'
      AND raw_point ->> 'altitude' IS NOT NULL
    THEN
      prev_altitude := (metric_anchor ->> 'altitude')::DOUBLE PRECISION;
      point_altitude := (raw_point ->> 'altitude')::DOUBLE PRECISION;
      delta_altitude := point_altitude - prev_altitude;
      IF delta_altitude >= 3 THEN
        total_ascent_m := total_ascent_m + round(delta_altitude)::INTEGER;
      ELSIF delta_altitude <= -3 THEN
