-- P1: reserve screenshot quota before OCR, refund only provider/transport/internal OCR failures.

BEGIN;

CREATE TABLE IF NOT EXISTS public.screenshot_quota_attempts (
  ...
  CONSTRAINT screenshot_quota_attempts_month_key_check
    CHECK (month_key ~ '^[0-9]{4}-[0-9]{2}$'),
  CONSTRAINT screenshot_quota_attempts_bucket_check
    CHECK (bucket IN ('free', 'paid')),
  CONSTRAINT screenshot_quota_attempts_status_check
    CHECK (status IN ('reserved', 'consumed', 'refunded'))
);

CREATE INDEX IF NOT EXISTS idx_screenshot_quota_attempts_user_month
  ON public.screenshot_quota_attempts(user_id, month_key, created_at);

ALTER TABLE public.screenshot_quota_attempts ENABLE ROW LEVEL SECURITY;

REVOKE ALL ON TABLE public.screenshot_quota_attempts FROM PUBLIC, anon, authenticated;
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public.screenshot_quota_attempts TO service_role;
...
BEGIN
  ...
  ELSIF locked_row.paid_used < p_paid_limit THEN
    UPDATE public.screenshot_quota
    SET paid_used = paid_used + 1, updated_at = now()
    WHERE id = locked_row.id
    RETURNING screenshot_quota.free_used, screenshot_quota.paid_used
    INTO free_used, paid_used;
    chosen_bucket := 'paid';
...
BEGIN
  SELECT *
  INTO attempt_row
  FROM public.screenshot_quota_attempts
  WHERE user_id = p_user_id
  ...
  IF attempt_row.status = 'consumed' THEN
    ...
    INTO free_used, paid_used
    FROM public.screenshot_quota q
...
BEGIN
  ...
  IF attempt_row.bucket = 'free' THEN
    ...
    WHERE user_id = p_user_id
      AND month_key = attempt_row.month_key
    RETURNING screenshot_quota.free_used, screenshot_quota.paid_used
    INTO free_used, paid_used;
  ELSE
    UPDATE public.screenshot_quota
    SET paid_used = GREATEST(paid_used - 1, 0), updated_at = now()
    WHERE user_id = p_user_id
      AND month_key = attempt_row.month_key
    RETURNING screenshot_quota.free_used, screenshot_quota.paid_used
    INTO free_used, paid_used;
