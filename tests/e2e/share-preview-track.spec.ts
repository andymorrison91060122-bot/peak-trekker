...
test('share editor renders empty, single-point, and real track previews without fake fallback routes', async ({
  ...
  const gpsShareLink = page.locator(`a[data-testid="profile-trip-share-link"][href="/share?checkinId=${multiCheckinId}"]`)
  await expect(gpsShareLink).toHaveText(/分享素材/)
  await captureOptionalE2EScreenshot(page, 'profile-realtime-gps-share-card.png')
  await gpsShareLink.click()
  await expect(page).toHaveURL(new RegExp(`/share\\?checkinId=${multiCheckinId}`), { timeout: 20_000 })
  const profileShareHero = page.getByTestId('share-hero-preview')
  await expect(profileShareHero).toBeVisible({ timeout: 20_000 })
  await expect(profileShareHero.locator('path[data-real-track="true"]')).toHaveCount(2)
  await expect(page.locator('body')).toContainText('GPS VERIFIED')
  await captureOptionalE2EScreenshot(page, 'profile-realtime-gps-share-editor.png')
})
