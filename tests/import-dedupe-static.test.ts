...
test('import client renders duplicate state with activity link and no duplicate confirm path', () => {
  ...
  assert.match(importClient, /router\.replace\(`\/activity\/\$\{checkinId\}`\)/)
  assert.doesNotMatch(importClient, /router\.push\(`\/activity\/\$\{checkinId\}`\)/)
})
...
test('import completed exits replace consumed flow history', () => {
  ...
  assert.doesNotMatch(importClient, /consumeImportCompletionExitRedirectFlag\(\)[\s\S]{0,120}router\.replace\('\/explore'\)/)
  assert.doesNotMatch(importClient, /if \(step === 'entry'\) \{[\s\S]{0,80}router\.back\(\)/)
})
