...
describe('FU-76 motion nodes Phase 2-I import and screenshot ceremonies', () => {
  ...
  test('explore motion is layered, limits first-screen card stagger, and does not alter list key structure', () => {
    ...
    assert.match(exploreClient, /const EXPLORE_BATCH_SIZE = 12/)
    assert.match(exploreClient, /const EXPLORE_BATCH_PRELOAD_OFFSET = 2/)
    assert.match(exploreClient, /filtered\.slice\(0, visibleCount\)/)
    assert.match(exploreClient, /new IntersectionObserver\([\s\S]*observer\.unobserve\(sentinel\)[\s\S]*currentCount \+ EXPLORE_BATCH_SIZE/)
    assert.match(exploreClient, /visibleResults\.map\(\(\{ mountain, length \}, index\) => \([\s\S]*<ExploreMountainCard[\s\S]*mountPending=\{index < 4 && !mountSettledRef\.current\}/m)
  ...
  test('FU-86 explore uses the approved scene panel, fixed filters, and real-only card meta', () => {
    ...
    assert.match(exploreMountainCard, /filterLengthKm: number \| null/)
    assert.match(exploreMountainCard, /data-length-km=\{filterLengthKm \?\? undefined\}/)
    assert.match(exploreMountainCard, /const displayLengthKm = getMountainDistanceKm\(mountain\)/)
    assert.match(exploreMountainCard, /const durationRange = getEstimatedDurationRange\(mountain\)/)
    assert.doesNotMatch(exploreMountainCard, /mountain\.estimated_duration\?\.trim\(\)/)
    assert.match(exploreMountainCard, /getExploreMountainThumbnailUrl\(heroImage\) \?\? EXPLORE_MOUNTAIN_COVER_FALLBACK_URL/)
    assert.match(exploreMountainCard, /<img[\s\S]*data-testid="explore-mountain-card-cover-image"[\s\S]*alt=\{mountain\.name\}/)
    assert.match(exploreMountainCard, /loading=\{imagePriority \? 'eager' : 'lazy'\}/)
    assert.match(exploreMountainCard, /fetchPriority=\{imagePriority \? 'high' : 'auto'\}/)
    assert.match(exploreMountainCard, /decoding="async"/)
    assert.match(exploreMountainCard, /const shouldLoadImage = imagePriority \|\| hasEnteredImageLoadRange/)
    assert.match(exploreMountainCard, /new IntersectionObserver\([\s\S]*rootMargin: '600px 0px'/)
    assert.match(exploreMountainCard, /src=\{shouldLoadImage \? thumbnailImage : undefined\}/)
    assert.match(exploreMountainCard, /image\.dataset\.fallbackAttempted === 'true'[\s\S]*image\.style\.visibility = 'hidden'/)
    assert.match(exploreMountainCard, /normalizedDifficulty === 'beginner' \? '入门线' : '进阶线'/)
    assert.match(exploreClient, /<ExploreMountainCard[\s\S]*filterLengthKm=\{length\}[\s\S]*mountPending=\{index < 4 && !mountSettledRef\.current\}[\s\S]*imagePriority=\{index < 2\}/)
    ...
    assert.match(componentsCss, /\.explore-card__cover \{[\s\S]*background-image: url\('\/images\/explore-mountain-cover-fallback\.webp'\)/)
    assert.match(componentsCss, /\.explore-card__cover-image \{[\s\S]*position: absolute;[\s\S]*width: 100%;[\s\S]*height: 100%;[\s\S]*object-fit: cover;/)
  ...
  test('FU-111 global press system is tokenized, consolidated, and avoids transform target conflicts', () => {
    ...
    assert.match(exploreMountainCard, /<Link[\s\S]*data-testid="explore-mountain-card"[\s\S]*style=\{\{ textDecoration: 'none', display: 'block', position: 'relative' \}\}/)
