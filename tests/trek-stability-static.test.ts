...
test('summit confirmed GSAP pilot owns ceremony motion and accessible count-up', () => {
  const summitConfirmedView = trekClient.match(/function SummitConfirmedView\([\s\S]*?function SummitHonorMedallion/)?.[0] ?? ''
  assert.match(trekClient, /import gsap from 'gsap'/)
  assert.match(trekClient, /import \{ useGSAP \} from '@gsap\/react'/)
  assert.match(trekClient, /gsap\.registerPlugin\(useGSAP\)/)
  assert.match(summitConfirmedView, /useGSAP\(\(\) =>/)
  assert.match(summitConfirmedView, /scope:\s*rootRef/)
  assert.match(summitConfirmedView, /gsap\.matchMedia\(\)/)
  assert.match(summitConfirmedView, /reduceMotion:\s*'\(prefers-reduced-motion: reduce\)'/)
  assert.match(summitConfirmedView, /duration:\s*1\.15[\s\S]{0,80}ease:\s*'power2\.out'/)
  assert.match(summitConfirmedView, /writeAltitude\(officialAltitude\)/)
  assert.match(summitConfirmedView, /data-testid="trek-summit-altitude"[\s\S]{0,80}aria-hidden="true"/)
  assert.match(summitConfirmedView, /className="sr-only"[\s\S]{0,80}\{officialAltitudeCaption\} \{officialAltitudeSemanticLabel\}/)
  assert.match(summitConfirmedView, /data-pt-summit-stats-card/)
  assert.match(summitConfirmedView, /\.to\(statsCard,[\s\S]{0,120}autoAlpha:\s*1[\s\S]{0,80}1\.15/)
  assert.match(summitConfirmedView, /const driftTimeline = gsap\.timeline\(\{ paused: true \}\)/)
  assert.match(summitConfirmedView, /driftTimeline\.play\(0\)/)
  assert.match(summitConfirmedView, /svgOrigin:\s*'86 86'/)
  assert.match(summitConfirmedView, /duration:\s*170[\s\S]{0,80}repeat:\s*-1/)
  assert.match(summitConfirmedView, /duration:\s*240[\s\S]{0,80}repeat:\s*-1/)
  assert.equal((summitConfirmedView.match(/mm\.revert\(\)/g) ?? []).length, 1)
  assert.doesNotMatch(summitConfirmedView, /tl\.call\(\(\) => \{[\s\S]{0,80}gsap\.to\(ring/)
  assert.doesNotMatch(summitConfirmedView, /transformBox:\s*'fill-box'/)
  assert.doesNotMatch(trekClient, /@keyframes pt-summit-honor-/)
  assert.doesNotMatch(summitConfirmedView, /animatedOfficialAltitude|prefersReducedMotion|requestAnimationFrame/)
  assert.doesNotMatch(trekClient, /ScrollTrigger/)
})
